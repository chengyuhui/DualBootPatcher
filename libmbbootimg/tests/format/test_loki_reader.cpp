...
TEST(LokiFindRamdiskAddressTest, NewImageValidShouldSucceed)
{
    ...
    data.insert(data.end(), LOKI_SHELLCODE,
                LOKI_SHELLCODE + LOKI_SHELLCODE_SIZE - 4);
    data.push_back(0xaa);
    data.push_back(0xbb);
    data.push_back(0xcc);
    data.push_back(0xdd);
...
TEST(LokiReadOldHeaderTest, ValidImageShouldSucceed)
{
    ...
    memcpy(data.data() + 3 * ahdr.page_size, LOKI_SHELLCODE,
           LOKI_SHELLCODE_SIZE);
    data[3 * ahdr.page_size + LOKI_SHELLCODE_SIZE - 4] = 0xaa;
    data[3 * ahdr.page_size + LOKI_SHELLCODE_SIZE - 3] = 0xbb;
    data[3 * ahdr.page_size + LOKI_SHELLCODE_SIZE - 2] = 0xcc;
    data[3 * ahdr.page_size + LOKI_SHELLCODE_SIZE - 1] = 0xdd;
