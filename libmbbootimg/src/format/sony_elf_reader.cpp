...
SonyElfReaderEntry * sony_elf_reader_next_entry(SonyElfReaderCtx *const ctx)
{
    if (ctx->entry->type == SONY_ELF_READER_ENTRY_END) {
        return ctx->entry;
    } else {
        return ctx->entry + 1;
    }
...
static int sony_elf_reader_add_entry(MbBiReader *bir, SonyElfReaderCtx *ctx,
                                     int type, uint64_t offset, uint32_t size)
{
    ...
    SonyElfReaderEntry *entry = &ctx->entries[ctx->entries_len];
    entry->type = type;
    entry->offset = offset;
    entry->size = size;
                                ...
                                MbBiHeader *header)
{
    ...
    // Beginning entry
    ret = sony_elf_reader_add_entry(bir, ctx, SONY_ELF_READER_ENTRY_BEGIN, 0, 0);
    if (ret != MB_BI_OK) return ret;
    ...
    for (Elf32_Half i = 0; i < ctx->hdr.e_phnum; ++i) {
        ...
        if (phdr.p_type == SONY_E_TYPE_CMDLINE
            ...
            ret = sony_elf_reader_add_entry(bir, ctx, MB_BI_ENTRY_KERNEL,
            ...
            ret = sony_elf_reader_add_entry(bir, ctx, MB_BI_ENTRY_RAMDISK,
                                            phdr.p_offset,phdr.p_memsz);
            if (ret != MB_BI_OK) return ret;
            ...
            ret = sony_elf_reader_add_entry(bir, ctx, MB_BI_ENTRY_SONY_IPL,
                                            phdr.p_offset,phdr.p_memsz);
            if (ret != MB_BI_OK) return ret;
            ...
            ret = sony_elf_reader_add_entry(bir, ctx, MB_BI_ENTRY_SONY_RPM,
                                            phdr.p_offset,phdr.p_memsz);
            if (ret != MB_BI_OK) return ret;
            ...
            ret = sony_elf_reader_add_entry(bir, ctx, MB_BI_ENTRY_SONY_APPSBL,
                                            phdr.p_offset,phdr.p_memsz);
            if (ret != MB_BI_OK) return ret;
    ...
    ret = sony_elf_reader_add_entry(bir, ctx, SONY_ELF_READER_ENTRY_END, 0, 0);
    if (ret != MB_BI_OK) return ret;
...
{
    SonyElfReaderCtx *const ctx = static_cast<SonyElfReaderCtx *>(userdata);
    SonyElfReaderEntry *rentry;
    uint64_t size;
    int ret;

    // Advance to next entry
    rentry = sony_elf_reader_next_entry(ctx);

    if (rentry->type == SONY_ELF_READER_ENTRY_END) {
        return MB_BI_EOF;
    ...
    // Check truncation here instead of in sony_elf_reader_read_data() so we can
    ...
    if (size > ctx->file_size || rentry->offset > ctx->file_size - size) {
    ...
    // Integer overflow already checked in sony_elf_reader_read_header()
    uint64_t read_start_offset = rentry->offset;
    uint64_t read_end_offset = read_start_offset + size;
