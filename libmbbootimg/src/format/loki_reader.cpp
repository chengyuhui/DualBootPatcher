...
LokiReaderEntry * loki_reader_next_entry(LokiReaderCtx *const ctx)
{
    if (ctx->entry->type == LOKI_READER_ENTRY_END) {
        return ctx->entry;
    } else {
        return ctx->entry + 1;
    }
                              ...
                              uint32_t *ramdisk_addr_out)
{
    ...
    if (loki_hdr->ramdisk_addr != 0) {
        ...
        ret = mb_file_search(file, -1, -1, 0, LOKI_SHELLCODE,
                             LOKI_SHELLCODE_SIZE - 8, 1, result_cb, &offset);
        if (ret < 0) {
            mb_bi_reader_set_error(bir, mb_file_error(file),
...
static int loki_reader_add_entry(MbBiReader *bir, LokiReaderCtx *ctx,
                                 int type, uint64_t offset, uint32_t size)
{
    ...
    LokiReaderEntry *entry = &ctx->entries[ctx->entries_len];
    entry->type = type;
    entry->offset = offset;
    entry->size = size;
                            ...
                            MbBiHeader *header)
{
    ...
    ret = loki_reader_add_entry(bir, ctx, LOKI_READER_ENTRY_BEGIN, 0, 0);
    if (ret != MB_BI_OK) return ret;

    ret = loki_reader_add_entry(bir, ctx, MB_BI_ENTRY_KERNEL,
                                kernel_offset, kernel_size);
    if (ret != MB_BI_OK) return ret;

    ret = loki_reader_add_entry(bir, ctx, MB_BI_ENTRY_RAMDISK,
                                ramdisk_offset, ramdisk_size);
    if (ret != MB_BI_OK) return ret;

    if (ctx->hdr.dt_size > 0 && dt_offset != 0) {
        ret = loki_reader_add_entry(bir, ctx, MB_BI_ENTRY_DEVICE_TREE,
                                    dt_offset, ctx->hdr.dt_size);
        if (ret != MB_BI_OK) return ret;
    }

    ret = loki_reader_add_entry(bir, ctx, LOKI_READER_ENTRY_END, 0, 0);
    if (ret != MB_BI_OK) return ret;
...
{
    LokiReaderCtx *const ctx = static_cast<LokiReaderCtx *>(userdata);
    LokiReaderEntry *rentry;
    int ret;

    // Advance to next entry
    rentry = loki_reader_next_entry(ctx);

    if (rentry->type == LOKI_READER_ENTRY_END) {
        return MB_BI_EOF;
    ...
    // Check truncation here instead of in loki_reader_read_data() so we can
    ...
    // Integer overflow already checked in loki_reader_read_header()
