...
AndroidReaderEntry * android_reader_next_entry(AndroidReaderCtx *const ctx)
{
    if (ctx->entry->type == ANDROID_READER_ENTRY_END) {
        return ctx->entry;
    } else {
        return ctx->entry + 1;
    }
...
static int android_reader_add_entry(MbBiReader *bir, AndroidReaderCtx *ctx,
                                    int type, uint64_t offset, uint32_t size)
{
    ...
    AndroidReaderEntry *entry = &ctx->entries[ctx->entries_len];
    entry->type = type;
    entry->offset = offset;
    entry->size = size;
                               ...
                               MbBiHeader *header)
{
    ...
    ret = android_reader_add_entry(bir, ctx, ANDROID_READER_ENTRY_BEGIN, 0, 0);
    if (ret != MB_BI_OK) return ret;

    ret = android_reader_add_entry(bir, ctx, MB_BI_ENTRY_KERNEL,
                                   kernel_offset, ctx->hdr.kernel_size);
    if (ret != MB_BI_OK) return ret;

    ret = android_reader_add_entry(bir, ctx, MB_BI_ENTRY_RAMDISK,
                                   ramdisk_offset, ctx->hdr.ramdisk_size);
    if (ret != MB_BI_OK) return ret;

    if (ctx->hdr.second_size > 0) {
        ret = android_reader_add_entry(bir, ctx, MB_BI_ENTRY_SECONDBOOT,
                                       second_offset, ctx->hdr.second_size);
        if (ret != MB_BI_OK) return ret;
    }

    if (ctx->hdr.dt_size > 0) {
        ret = android_reader_add_entry(bir, ctx, MB_BI_ENTRY_DEVICE_TREE,
                                       dt_offset, ctx->hdr.dt_size);
        if (ret != MB_BI_OK) return ret;
    }

    ret = android_reader_add_entry(bir, ctx, ANDROID_READER_ENTRY_END, 0, 0);
    if (ret != MB_BI_OK) return ret;
...
{
    AndroidReaderCtx *const ctx = static_cast<AndroidReaderCtx *>(userdata);
    AndroidReaderEntry *rentry;
    uint64_t size;
    int ret;

    // Advance to next entry
    rentry = android_reader_next_entry(ctx);

    if (rentry->type == ANDROID_READER_ENTRY_END) {
        return MB_BI_EOF;
    ...
    if (size > ctx->file_size || rentry->offset > ctx->file_size - size) {
        ...
        if (rentry->type != MB_BI_ENTRY_DEVICE_TREE
                || !ctx->allow_truncated_dt) {
            mb_bi_reader_set_error(bir, MB_BI_ERROR_FILE_FORMAT,
                                   "Image is truncated "
                                   "(expected %" PRIu64 " more bytes)",
                                   ctx->read_end_offset - ctx->read_cur_offset);
        ...
        size = ctx->file_size - rentry->offset;
    }
