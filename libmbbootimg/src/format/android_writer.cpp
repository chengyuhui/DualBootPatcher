...
{
    if (ctx->entry->type != ANDROID_WRITER_ENTRY_END) {
        ++ctx->entry;
    }
}
...
static int android_writer_add_entry(MbBiWriter *biw, AndroidWriterCtx *ctx,
                                    int type, uint32_t size, bool size_set)
{
    ...
    AndroidWriterEntry *entry = &ctx->entries[ctx->entries_len];
    entry->type = type;
    entry->size = size;
                                ...
                                MbBiHeader *header)
{
    ...
    ret = android_writer_add_entry(biw, ctx, ANDROID_WRITER_ENTRY_BEGIN, 0, false);
    if (ret != MB_BI_OK) return ret;

    ret = android_writer_add_entry(biw, ctx, MB_BI_ENTRY_KERNEL, 0, false);
    if (ret != MB_BI_OK) return ret;

    ret = android_writer_add_entry(biw, ctx, MB_BI_ENTRY_RAMDISK, 0, false);
    if (ret != MB_BI_OK) return ret;

    ret = android_writer_add_entry(biw, ctx, MB_BI_ENTRY_SECONDBOOT, 0, false);
    if (ret != MB_BI_OK) return ret;

    ret = android_writer_add_entry(biw, ctx, MB_BI_ENTRY_DEVICE_TREE, 0, false);
    if (ret != MB_BI_OK) return ret;

    ret = android_writer_add_entry(biw, ctx, ANDROID_WRITER_ENTRY_END, 0, false);
    if (ret != MB_BI_OK) return ret;
                             ...
                             MbBiEntry **entry)
{
    ...
    if (ctx->entry->type >= 0) {
        ...
        android_writer_update_size_if_unset(ctx, ctx->entry->size);
...
int android_writer_close(MbBiWriter *biw, void *userdata)
{
    ...
    if (ctx->entry->type == ANDROID_WRITER_ENTRY_END) {
        ...
        if (ctx->is_bump) {
            ret = mb_file_write_fully(biw->file, BUMP_MAGIC,
                                      BUMP_MAGIC_SIZE, &n);
            ...
            ret = mb_file_write_fully(biw->file, SAMSUNG_SEANDROID_MAGIC,
        ...
        for (size_t i = 0; i < ctx->entries_len; ++i) {
            AndroidWriterEntry *entry = &ctx->entries[i];
            if (entry->size_set) {
                switch (entry->type) {
